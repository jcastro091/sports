...
def compute_tier(p_ml: float, edge: float, drop_cents: int, threshold_cents: int, limit_trend: str) -> tuple[str, str]:
    """
    Returns (tier_code, label) where:
      tier_code in {"A","B","C","PASS"}
      label is the colored text for Telegram (e.g., "🟢 A+ Strong", "🟡 B-Tier Value", "🟠 C-Tier Watchlist", "⚫ Pass")
    """
    lt = (limit_trend or "flat").lower()

    # A+: high confidence
    if (p_ml >= TIER_A_MIN_PROBA and
        edge >= TIER_A_MIN_EDGE and
        drop_cents >= max(TIER_A_MIN_REV_CENTS, threshold_cents) and
        lt in ("flat", "up")):
        return "A", "🟢 A+ Strong"

    # B: medium confidence
    if (p_ml >= TIER_B_MIN_PROBA and
        edge >= TIER_B_MIN_EDGE and
        drop_cents >= max(TIER_B_MIN_REV_CENTS, threshold_cents) and
        lt in ("flat", "up")):
        return "B", "🟡 B-Tier Value"

    # C: data/watchlist (light or no-bet)
    if (p_ml >= TIER_C_MIN_PROBA and
        edge >= TIER_C_MIN_EDGE and
        drop_cents >= max(TIER_C_MIN_REV_CENTS, min(threshold_cents, TIER_C_MIN_REV_CENTS))):
        return "C", "🟠 C-Tier Watchlist"
                ...
                for game in all_events:
                    ...
                    for key, label in price_keys.items():
                        ...
                        if obs_sheet is not None:
                            ...
                            # ===== Sheets: AllObservations (pre-send path; only used if RECORD_ON_SEND_ONLY == 0) =====
                            if obs_sheet is not None and not RECORD_ON_SEND_ONLY:
                                ...
                                # --- New: compute tier once, then log both code & label
                                tier_code, tier_label = compute_tier(
                                    p_ml=best["p_ml"],
                                    edge=best["edge"],
                                    drop_cents=best.get("drop_cents", 0),
                                    threshold_cents=best.get("threshold_cents", 0),
                                    limit_trend=best.get("trend", "flat"),
                                )
                                ...
                                gid        = best["game_id"]
                                opening_val = peak_prices[gid].get(f"opening_{best['market_key']}")
                                peak_val    = peak_prices[gid].get(best["market_key"])
                                ...
                                obs_map = {
                                    ...
                                    "Away": best["parsed"]["away_team"],
                                    "Home": best["parsed"]["home_team"],
                                    "Market": label,
                                    "Direction": best["ml_direction"],                       # you use 'Direction' earlier in header
                                    "Movement": best["movement_str"],
                                    "Spread Line Home": best["parsed"].get("spread_line_home") or "",
                                    "Spread Line Away": best["parsed"].get("spread_line_away") or "",
                                    "Total Line": best["parsed"].get("total_line") or "",
                                    "Odds (Am)": am_or_blank(curr_dec),                      # fills your “Odds (Am)” column
                                    "Reason Text": _rule_based_reason(label, best["p_ml"], curr_dec, best["movement_str"], best["limit_now"], best["trend"]),
                                    "Opening Decimal": opening_val,
                                    "Peak Decimal": peak_val,
                                    "Current Decimal": float(curr_dec),
                                    "MinutesToStart": round(
                                        (parse_commence_utc(tracked_games[gid]["commence_time"]) - datetime.now(timezone.utc)).total_seconds()/60.0, 1
                                    ),
                                    "Actual Winner": "",                                     # keep blank per your schema
                                    "Predicted": best["predicted_value"],
                                    "Prediction Result": "",                                 # keep blank per your schema
                                    "Stake Amount": f"{best['stake_amount']:.2f}",
                                    "Game Time": best["game_time_fmt"],
                                    "American Odds": am_or_blank(curr_dec),                  # you also have this column; okay to duplicate
                                    ...
                                    "Edge": f"{best['edge']:.6f}",
                                    "Bet ID": best["bet_id"],                                # lands in the correct column now
                                    # Blank 3 & Blank4 will become Tier columns after you rename them:
                                    "Tier Code": tier_code,                                  # <- rename Blank 3 header to this
                                    "Tier Label": tier_label,                                # <- rename Blank4 header to this
                                    "Blank5": "", "Blank6": "",
                                    "ML Direction": best["ml_direction"],                    # you also keep this later in sheet
                                    ...
                                    # The remaining blanks and Kalshi fields are preserved as empty for now:
