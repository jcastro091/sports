...
def main():
    ...
    # --- MLflow logging (SageMaker) ---
    tracking_uri = os.getenv("MLFLOW_TRACKING_URI")
    if not tracking_uri:
        raise RuntimeError(
            "MLFLOW_TRACKING_URI is not set. Make sure the SageMaker "
            "estimator passes the managed MLflow tracking server ARN."
        )

    experiment_name = os.getenv("MLFLOW_EXPERIMENT_NAME", "sharpsignal-sagemaker")

    mlflow.set_tracking_uri(tracking_uri)
    mlflow.set_experiment(experiment_name)


    # Use the SageMaker training job name as run name
