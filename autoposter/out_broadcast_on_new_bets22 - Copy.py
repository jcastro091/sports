    ...
    while True:
        ...
        try:
            ...
            for _, row in df2.iterrows():
                ...
                bet_id = n.get("bet_id","")
                if not bet_id:
                    logger.debug("Row without bet_id skipped.")
                    continue
                ...
                if X_ENABLED and x_client and can_retry(bet_id) and (int(time.time()) >= _X_LOCK_UNTIL):
                    try:
                        ...
                        if tweet_id:
                            ...
                            # Treat empty id as rate-limit or transient; back off globally ~15 min
                            logger.error("X post returned empty tweet_id (bet_id=%s). Applying global lockout.", bet_id)
                            set_retry_after(bet_id, 5 * 60)       # per-bet
                            _X_LOCK_UNTIL = int(time.time()) + 15 * 60  # global
            ...
            # Only advance cursor if we actually posted, or if we processed all rows without deferring
            if not broke_for_rate_limit and (posted > 0 or len(df2) == 0):
                if parsed_all:
                    try:
                        state["last_seen_iso"] = str(max([p for p in parsed_all if p is not None]))
                    except Exception:
