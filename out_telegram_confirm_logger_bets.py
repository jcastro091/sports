...
HC_URL = os.getenv("HEARTBEAT_URL_TG_BETS", "").strip()
HC_FILE = os.getenv("HEARTBEAT_FILE_TG_BETS", "").strip()
_last_hb_bucket = -1
...
def _hb_ping(suffix: str = "", payload: dict | None = None):
    ...
    try:
        if payload:
            requests.post(u, json=payload, timeout=5)
        else:
...
def _hb_tick():
    ...
    if b != _last_hb_bucket:
        _last_hb_bucket = b
        _hb_ping("", {"svc":"tg_bets","ts": int(time.time())})
        _hb_touch()
# ========= CONFIG =========
...
# Start heartbeat
try:
    _hb_ping("start", {"svc":"tg_bets","status":"start","ts":int(time.time())})
    _hb_touch()
except Exception:
    pass
...
threading.Thread(target=_hb_background, daemon=True).start()
