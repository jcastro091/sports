...
# - NEW (2025-09-17): Single-alert flow using pos/price to infer Entry vs Exit; side fallback from pos;
#                     optional synthetic trade_id when not provided.
...
def update_or_append(ws, data: dict):
    ...
    side      = (data.get("side") or "").strip().lower()  # "long" / "short" (may be blank/other)
    ...
    # NEW: helper fields to infer entry/exit from a single alert
    pos       = float(data.get("pos") or 0)       # signed position size from Pine
    price_now = float(data.get("price") or 0)     # current price (used as entry/exit fallback)

    # NEW: if side is missing/ambiguous, infer from pos
    ...
    # NEW: if no explicit event, infer it from pos
    # - pos == 0  -> EXIT, use price_now as exit if exit not provided
    # - pos != 0  -> ENTRY/refresh, use price_now as entry if entry not provided
    if not event:
    ...
    # NEW (optional): if no trade_id provided, synthesize a stable-ish one
    if not trade_id and symbol and side and entry:
        trade_id = f"{symbol}-{side}-{round(float(entry), 2)}-{interval or 'n/a'}"
...
def tv_webhook():
    try:
        ...
        # Compose Telegram message (infer EXIT if pos==0 even when event is blank)
        ...
        exit_px  = data.get("exit", "") or data.get("price", "")
        sl       = data.get("sl", "")
        tp       = data.get("tp", "")
        rr       = data.get("rr", "")
        tag      = data.get("tag", "")
